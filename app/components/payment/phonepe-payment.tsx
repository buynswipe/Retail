...
import { initiatePhonePePayment } from "@/lib/payment-service"
...
export function PhonePePayment({
  ...
  const handlePayment = async () => {
    try {
      ...
      if (response.success) {
        // For PhonePe, we would typically redirect to their payment page
        // But since we're deprecating it in favor of PayU, we'll just show a message
        setIsLoading(false)
        toast({
          title: "Payment Method Deprecated",
          description: "PhonePe payments are being phased out. Please use PayU instead.",
          variant: "destructive",
        })
        onFailure("PhonePe payments are being phased out. Please use PayU instead.")
      } else {
        setIsLoading(false)
        toast({
          title: "Payment Error",
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Pay using PhonePe</CardDescription>
      </CardHeader>
      <CardContent>
        <Alert className="bg-amber-50 border-amber-200">
          <AlertDescription className="text-amber-800">
            PhonePe payments are being phased out. We recommend using PayU for the best experience.
          </AlertDescription>
        </Alert>
