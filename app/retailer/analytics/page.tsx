...
import { ShoppingBag, IndianRupee, TrendingUp, Filter, X } from "lucide-react"
...
function RetailerAnalyticsDashboardContent() {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      const { data: ordersByStatus } = await supabase.rpc("get_retailer_orders_by_status_with_filters", {
        retailer_id: user.id,
      ...
      // Get orders by day
      const { data: ordersByDay } = await supabase.rpc("get_retailer_orders_by_day_with_filters", {
        retailer_id: user.id,
        timeframe_filter: timeframe,
        date_range_start: dateRange?.from?.toISOString(),
        date_range_end: dateRange?.to?.toISOString(),
      ...
      // Get spending by day
      const { data: spendingByDay } = await supabase.rpc("get_retailer_spending_by_day_with_filters", {
        retailer_id: user.id,
        timeframe_filter: timeframe,
        date_range_start: dateRange?.from?.toISOString(),
        date_range_end: dateRange?.to?.toISOString(),
      ...
      const { data: topProducts } = await supabase.rpc("get_retailer_top_products", {
        retailer_id: user.id,
        timeframe_filter: timeframe,
        date_range_start: dateRange?.from?.toISOString(),
        date_range_end: dateRange?.to?.toISOString(),
