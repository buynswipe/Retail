...
export async function verifyPayment(params: VerifyPaymentParams): Promise<VerifyPaymentResponse> {
  try {
    ...
    switch (gateway) {
      case "razorpay":
        isValid = await verifyRazorpayPayment(orderId, gatewayPaymentId, signature || "")
        break
      case "paytm":
        isValid = await verifyPaytmPayment(responseData)
        break
      case "phonepe":
        isValid = await verifyPhonePePayment(responseData)
        break
      case "payu":
        isValid = await verifyPayUPayment(responseData)
        break
      case "cod":
...
async function verifyRazorpayPayment(orderId: string, paymentId: string, signature: string): Promise<boolean> {
  try {
    // Get the Razorpay order ID from our database
    const supabase = createClient()
    const { data, error } = await supabase.from("payments").select("payment_details").eq("order_id", orderId).single()

    if (error || !data || !data.payment_details || !data.payment_details.razorpay_order_id) {
      return false
    }

    const razorpayOrderId = data.payment_details.razorpay_order_id
...
async function verifyPaytmPayment(responseData: Record<string, any>): Promise<boolean> {
  try {
    // In a real implementation, you would verify the checksum
    // For now, we'll just check if the status is successful
...
async function verifyPhonePePayment(responseData: Record<string, any>): Promise<boolean> {
  try {
    // In a real implementation, you would verify the signature
    // For now, we'll just check if the code is successful
...
async function verifyPayUPayment(responseData: Record<string, any>): Promise<boolean> {
  try {
...
export async function createPaymentOrder(
  ...
  try {
    ...
    switch (gateway) {
      case "razorpay":
        return await createRazorpayOrder(payment.id, orderId, amount, currency)
      case "paytm":
        return await createPaytmOrder(payment.id, orderId, amount, currency)
      case "phonepe":
        return await createPhonePeOrder(payment.id, orderId, amount, currency)
      case "payu":
        return await createPayUOrder(payment.id, orderId, amount, currency)
      case "cod":
...
async function createRazorpayOrder(
  ...
  return { success: true, data: { paymentId, method: "razorpay" } }
}

async function createPaytmOrder(
  paymentId: string,
  orderId: string,
  amount: number,
  currency: string,
  ...
  return { success: true, data: { paymentId, method: "paytm" } }
}
...
async function createPhonePeOrder(
  ...
  return { success: true, data: { paymentId, method: "phonepe" } }
}
...
async function createPayUOrder(
  ...
  return { success: true, data: { paymentId, method: "payu" } }
}
